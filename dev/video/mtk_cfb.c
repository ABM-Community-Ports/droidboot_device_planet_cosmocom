...
static void cfb_lk_dchars(int xx, int yy, unsigned char *s, int count)
{
	...
	// dprintf(CRITICAL, "CFB_LK_DCHARS %d %d %d\n", xx, yy, data_fmt);
	...
	case CFB_X888RGB_32BIT:
	  // USED BY COSMO LCD
		// dprintf(CRITICAL, "VDO_COLS is %d, PIXEL_SIZE is %d, LINE_LEN is %d, ROTATION is %s\n", VDO_COLS, PIXEL_SIZE, LINE_LEN, MTK_LCM_PHYSICAL_ROTATION);
		while (count--) {
			offs = (*s++) * MTK_VFH;
			pos = pfont + offs;
			row = MTK_VFH;
			if (0 == strncmp(MTK_LCM_PHYSICAL_ROTATION, "90", 2)) {
				...
				// THIS IS THE DEFAULT ROTATION FOR COSMO
				for (tdest = pdest; row--; tdest += PIXEL_SIZE) {
					unsigned char bits = *pos++;
					((unsigned int *)tdest)[15 * VDO_COLS + MTK_VFH * VDO_COLS] =
							0xff000000 |
							SWAP32((lk_cfb_font_dtable32[bits >> 4][0] & cfb_eorx) ^
							 cfb_bgx);
					((unsigned int *)tdest)[14 * VDO_COLS + MTK_VFH * VDO_COLS] =
							0xff000000 |
							SWAP32((lk_cfb_font_dtable32[bits >> 4][0] & cfb_eorx) ^
							 cfb_bgx);

					((unsigned int *)tdest)[13 * VDO_COLS + MTK_VFH * VDO_COLS] =
							0xff000000 |
							SWAP32((lk_cfb_font_dtable32[bits >> 4][1] & cfb_eorx) ^
							 cfb_bgx);
					((unsigned int *)tdest)[12 * VDO_COLS + MTK_VFH * VDO_COLS] =
							0xff000000 |
							SWAP32((lk_cfb_font_dtable32[bits >> 4][1] & cfb_eorx) ^
		 					 cfb_bgx);

					((unsigned int *)tdest)[11 * VDO_COLS + MTK_VFH * VDO_COLS] =
							0xff000000 |
							SWAP32((lk_cfb_font_dtable32[bits >> 4][2] & cfb_eorx) ^
							 cfb_bgx);
					((unsigned int *)tdest)[10 * VDO_COLS + MTK_VFH * VDO_COLS] =
							0xff000000 |
							SWAP32((lk_cfb_font_dtable32[bits >> 4][2] & cfb_eorx) ^
							 cfb_bgx);

					((unsigned int *)tdest)[9 * VDO_COLS + MTK_VFH * VDO_COLS] =
							0xff000000 |
							SWAP32((lk_cfb_font_dtable32[bits >> 4][3] & cfb_eorx) ^
							 cfb_bgx);
					((unsigned int *)tdest)[8 * VDO_COLS + MTK_VFH * VDO_COLS] =
							0xff000000 |
							SWAP32((lk_cfb_font_dtable32[bits >> 4][3] & cfb_eorx) ^
							 cfb_bgx);
...
void video_printf(const char *fmt, ...)
{
  ...
  // dprintf(CRITICAL, "VIDEO_PRINTF %s\n", fmt);
