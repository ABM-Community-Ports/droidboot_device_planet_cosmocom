...
static void set_ccb_gear_val(char* env_name, unsigned int gear_id)
{
	int ret;
	char env_buf[12];
	...
	ret = set_env(env_name, env_buf);
	if (ret < 0) {
		ALWAYS_LOG("set env[%s]fail, ret=%d\n", env_name, ret);
		assert(0);
	} else
		ALWAYS_LOG("set env[%s]%d\n", env_name, gear_id);
}

static unsigned int get_ccb_gear_val(void)
{
	unsigned int meta_flag = 0x8000;
	unsigned int md1_ccb_cap_gear;
	unsigned int backup_ccb_cap_gear;
	char* md1_ccb_gear = "md1_ccb_cap_gear";
	char* backup_ccb_gear = "backup_ccb_cap_gear";

	md1_ccb_cap_gear = str2uint(get_env(md1_ccb_gear));
	ALWAYS_LOG("get env[%s]%d\n", md1_ccb_gear, md1_ccb_cap_gear);
	if (g_boot_mode == META_BOOT || true == get_atm_enable_status()) {
		ALWAYS_LOG("meta mode[%s]%d\n", md1_ccb_gear, CCB_DATA_BUF_DEFAULT_GEAR);
		backup_ccb_cap_gear = meta_flag | md1_ccb_cap_gear;
		set_ccb_gear_val(backup_ccb_gear, backup_ccb_cap_gear);
		set_ccb_gear_val(md1_ccb_gear, CCB_DATA_BUF_DEFAULT_GEAR);
		return CCB_DATA_BUF_DEFAULT_GEAR;
	}
	backup_ccb_cap_gear = str2uint(get_env(backup_ccb_gear));
	/* recovery md1_ccb_cap_gear if once enter meta mode */
	if ((backup_ccb_cap_gear & meta_flag) == meta_flag) {
		backup_ccb_cap_gear &= (~meta_flag);
		set_ccb_gear_val(backup_ccb_gear, backup_ccb_cap_gear);
		if (md1_ccb_cap_gear != backup_ccb_cap_gear) {
			md1_ccb_cap_gear = backup_ccb_cap_gear;
			ALWAYS_LOG("recovery ccb gear:%d\n", md1_ccb_cap_gear);
			if (md1_ccb_cap_gear != 0)
				set_ccb_gear_val(md1_ccb_gear, md1_ccb_cap_gear);
		}
	}
	if (md1_ccb_cap_gear == 0) {
