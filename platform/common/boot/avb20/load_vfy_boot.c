...
static uint32_t boot_post_processing(uint32_t bootimg_type,
				     AvbSlotVerifyData *slot_data)
{
	uint32_t ret = 0;
	uint32_t kimg_load_addr = 0;
	uint32_t i = 0;
	...
	for (i = 0; i < slot_data->num_loaded_partitions; i++) {
		if (!strcmp(part_name,
			...
			memcpy((void *)kimg_load_addr,
			       (void *)((uint8_t *)slot_data->loaded_partitions[i].data + get_page_sz()),
			       (size_t)(get_kernel_sz() + get_ramdisk_sz()));
			break;
		}
	}
	...
	ret = avb_cmdline_postprocessing(slot_data->cmdline, g_fdt);
	if (ret)
		goto end;
...
#if defined(MTK_DM_VERITY_OFF) || defined(MTK_BUILD_DEFAULT_UNLOCK)
