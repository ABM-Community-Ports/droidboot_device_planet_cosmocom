...
{
	int offset, val;
	offset = fdt_node_offset_by_compatible(g_fdt, -1, "mediatek,lk_charger");
	...
	if (offset >= 0) {
		...
		val = chr_fdt_getprop_u32(g_fdt, offset, "ac_charger_current");
		if (val)
			chr_cust_data.ac_charger_current = val / 1000;

		val = chr_fdt_getprop_u32(g_fdt, offset, "ac_charger_input_current");
		if (val)
			chr_cust_data.ac_charger_input_current = val / 1000;

		val = chr_fdt_getprop_u32(g_fdt, offset, "non_std_ac_charger_current");
		if (val)
		...
		val = chr_fdt_getprop_bool(g_fdt, offset, "enable_anime");
		if (val)
			chr_cust_data.enable_anime = true;
		else
			chr_cust_data.enable_anime = false;

		val = chr_fdt_getprop_u32(g_fdt, offset, "led_brightness");
		if (val)
			chr_cust_data.led_brightness = val;

		val = chr_fdt_getprop_u32(g_fdt, offset, "blinking_times");
		if (val)
			chr_cust_data.blinking_times = val;

		val = chr_fdt_getprop_u32(g_fdt, offset, "blinking_period");
		if (val)
			chr_cust_data.blinking_period = val;

		val = chr_fdt_getprop_u32(g_fdt, offset, "r_charger_1");
		if (val)
			chr_cust_data.r_charger_1 = val;

		val = chr_fdt_getprop_u32(g_fdt, offset, "r_charger_2");
		if (val)
...
#if defined(MACH_TYPE_MT6765) || defined(MACH_TYPE_MT6761)
...
static void check_charger_battery_on(void)
{
	...
	if (!is_battery_on()) {
		dprintf(CRITICAL, "[BATTERY] common No battry plug-in. Power Off.");
		//chr_power_off();
	}
}
...
void mtk_charger_start(void)
{
	...
	if (chr_cust_data.disable_charger == false) {
		...
		if (g_boot_mode != META_BOOT && g_boot_mode != FACTORY_BOOT && g_boot_mode != ATE_FACTORY_BOOT) {
			if (!is_battery_on()) {
				dprintf(CRITICAL, "[BATTERY] common2 No battry plug-in. Power Off.");
				//chr_power_off();
			}
		}
