...
static u32 ufs_query_desc_max_size[] = {
	...
	QUERY_DESC_CONFIGURAION_MAX_SIZE_HPB,
...
__WEAK int ufs_get_platform_data(void)
{
...
static int ufshcd_probe_hba(struct ufs_hba *hba)
{
	...
	if ((0 == hba->dev_info.bootable) ||
		(UFS_ERR_INVALID_LU_CONFIGURATION == ret))    /* new UFS or invalid LU configuration is found, start (re-)configuration flow */
				  ...
				  u32 param_size)
{
	...
	if (desc_len < (param_offset + param_size))
		return -1;

	if ((desc_len == param_size) && (!param_offset)) {
		/* memory space already available to hold full descriptor */
		desc_buf = param_read_buf;
	} else {
		/* allocate memory to hold full descriptor */
		if (sizeof(g_ufs_temp_buf) < desc_len) {
			UFS_DBG_LOGE("[UFS] err: ufshcd_read_desc_param - insufficient resp_data buffer size\n");
			return -1;
		}
	...
	if (ret ||
		...
		UFS_DBG_LOGE("[UFS] failed reading descriptor. desc_id %d param_offset %d desc_len %d ret %d\n", desc_id, param_offset, desc_len, ret);
...
int ufs_aio_check_lu_cfg(struct ufs_hba *hba)
{
	...
	if (hba->dev_info.hpb_support)
		ret = hba->read_descriptor(hba, QUERY_DESC_IDN_CONFIGURATION, 0, 1, (u8 *)&g_ufs_temp_buf[0], QUERY_DESC_CONFIGURAION_MAX_SIZE_HPB);
	else
		ret = hba->read_descriptor(hba, QUERY_DESC_IDN_CONFIGURATION, 0, 0, (u8 *)&g_ufs_temp_buf[0], QUERY_DESC_CONFIGURAION_MAX_SIZE);
	...
	if (hba->dev_info.hpb_support)
		ufs_aio_util_dump_table_hex(&g_ufs_temp_buf[0], QUERY_DESC_CONFIGURAION_MAX_SIZE_HPB);
	else
		ufs_aio_util_dump_table_hex(&g_ufs_temp_buf[0], QUERY_DESC_CONFIGURAION_MAX_SIZE);
...
#ifdef UFS_AIO_DEV_CHECK_LU_SIZE
	ret = hba->read_descriptor(hba, QUERY_DESC_IDN_GEOMETRY, 0, (u8 *)&g_ufs_temp_buf[0], QUERY_DESC_GEOMETRY_MAX_SIZE);
	...
	ufs_aio_util_dump_table_hex(&g_ufs_temp_buf[0], QUERY_DESC_GEOMETRY_MAX_SIZE);
...
int ufs_aio_configure_new_ufs(struct ufs_hba *hba)
{
	...
	if (hba->dev_info.hpb_support)
		ret = hba->read_descriptor(hba, QUERY_DESC_IDN_GEOMETRY, 0, 1, (u8 *)&g_ufs_temp_buf[0], QUERY_DESC_GEOMETRY_MAX_SIZE);
	else
		ret = hba->read_descriptor(hba, QUERY_DESC_IDN_GEOMETRY, 0, 0, (u8 *)&g_ufs_temp_buf[0], QUERY_DESC_GEOMETRY_MAX_SIZE);
	...
	if (hba->dev_info.hpb_support) {
		info.w_device_max_active_hpb_regisons = (p[0x4B] << 8) | p[0x4C];

		memset(&g_ufs_temp_buf[0], 0, QUERY_DESC_CONFIGURAION_MAX_SIZE_HPB);
		p[CONF_DESC_B_LENGTH]       = QUERY_DESC_CONFIGURAION_MAX_SIZE_HPB;
	} else {
	...
	for (i = 0; i < 8; i++) { /* traverse 8 Unit Descriptor configurable parameters */
		...
		if ((i == 2) && (hba->dev_info.hpb_support)) {
			...
			p[CONF_DESC_UNIT_W_MAX_ACTIVE_HPB_REGIONS] = (info.w_device_max_active_hpb_regisons >> 8) & 0xFF;
			p[CONF_DESC_UNIT_W_MAX_ACTIVE_HPB_REGIONS + 1] = info.w_device_max_active_hpb_regisons & 0xFF;
	...
	/* HPB support, write configuration descriptor with HPB, selector = 1, length = 0xD0 */
	if (hba->dev_info.hpb_support) {
		ufs_aio_util_dump_table_hex(&g_ufs_temp_buf[0], QUERY_DESC_CONFIGURAION_MAX_SIZE_HPB);
		ret = hba->write_descriptor(hba, QUERY_DESC_IDN_CONFIGURATION, 0, 1, &g_ufs_temp_buf[0], QUERY_DESC_CONFIGURAION_MAX_SIZE_HPB);
	} else {
		ufs_aio_util_dump_table_hex(&g_ufs_temp_buf[0], QUERY_DESC_CONFIGURAION_MAX_SIZE);
		ret = hba->write_descriptor(hba, QUERY_DESC_IDN_CONFIGURATION, 0, 0, &g_ufs_temp_buf[0], QUERY_DESC_CONFIGURAION_MAX_SIZE);
	}
...
int ufs_aio_get_device_info(struct ufs_hba *hba)
{
	...
	err = hba->read_descriptor(hba, QUERY_DESC_IDN_DEVICE, 0, 0, (u8 *)&g_ufs_temp_buf[0], QUERY_DESC_DEVICE_MAX_SIZE);
	...
	UFS_DBG_LOGE("[UFS] %s: Device Desc:\n", __func__);
	ufs_aio_util_dump_table_hex(p, QUERY_DESC_DEVICE_MAX_SIZE);
	...
	/*
	 * HPB support, re-read device descriptor with HPB
	 * selector = 1, length = 0x48
	 */
	if (card_data->hpb_support) {
		...
		UFS_DBG_LOGD("HPB Version = %x %x\n",
			(card_data->hpb_ver >> 8) & 0xFF,
			card_data->hpb_ver & 0xFF);
	}
	...
	if (err) {
		...
		UFS_DBG_LOGD("[UFS] %s: String Desc: Serial Number INDEX %d, LENGTH %d\n",
			__func__, sn, card_data->serial_number_len);
		...
		if (card_data->serial_number_len < MAX_SERAL_NUMBER_LEN) {
			...
			UFS_DBG_LOGD("[UFS] Serial Number: %s\n", card_data->serial_number);
		} else {
	...
	/* FW 0610 is test version, may not stable, disable hpb and use default setting */
	if (strcmp(card_data->product_revision_level, "0610") == 0) {
		card_data->hpb_support = 0;
		card_data->ud0_base_offset = 0x10;
		card_data->ud_config_len = 0x10;
		UFS_DBG_LOGD("FW ver : %s, disable HPB\n", card_data->product_revision_level);
	}
...
int ufs_aio_read_unit_desc_cfg_param(struct ufs_hba *hba)
{
	...
	if (((p[0] != QUERY_DESC_CONFIGURAION_MAX_SIZE) && (p[0] != QUERY_DESC_CONFIGURAION_MAX_SIZE_HPB)) ||
		(p[1] != 0x1)) {
