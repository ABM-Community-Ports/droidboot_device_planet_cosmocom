        ...
        enum enum_rpmb_msg type)
{
	...
	u8 rpmb_hmac[32];
	u8 *buf = NULL;
	u8 rpmb_hmac_data[HMAC_TEXT_LEN];
#endif //CFG_RPMB_KEY_RETRY_IN_PL
	if (cmd == NULL) {
		UFS_DBG_LOGE("[%s]: RPMB cmd pointer NULL\n", __func__);
		return -1;
	...
	memcpy(buf, rpmb_frame->data, RPMB_FRM_DATA_LEN);
	buf += RPMB_FRM_DATA_LEN;

	memcpy(buf, rpmb_frame->nonce, RPMB_FRM_NONCE_LEN);
	buf += RPMB_FRM_NONCE_LEN;

	memcpy(buf, &rpmb_frame->wr_cnt, RPMB_FRM_WR_CNT_LEN);
	buf += RPMB_FRM_WR_CNT_LEN;

	memcpy(buf, &rpmb_frame->addr, RPMB_FRM_ADDR_LEN);
	buf += RPMB_FRM_ADDR_LEN;

	memcpy(buf, &rpmb_frame->blk_cnt, RPMB_FRM_BLK_CNT_LEN);
	buf += RPMB_FRM_BLK_CNT_LEN;

	memcpy(buf, &rpmb_frame->result, RPMB_FRM_RESULT_LEN);
	buf += RPMB_FRM_RESULT_LEN;

	memcpy(buf, &rpmb_frame->req_resp, RPMB_FRM_REQ_RESP_LEN);
	buf += RPMB_FRM_REQ_RESP_LEN;

	ret = hmac_sha256(g_rpmb_key, 32, rpmb_hmac_data, HMAC_TEXT_LEN, rpmb_hmac);
	if (ret) {
		UFS_DBG_LOGE("%s, rpmb_hmac error.\n", __func__);
		return -1;
	}

	if (memcmp(rpmb_frame->key_MAC, rpmb_hmac, 32) != 0) {
...

{
	...
	UFS_DBG_LOGE("[%s]: Dump cfg block before writing to device:\n", __func__);
...
int ufs_set_write_protect(int dev_num, ufs_logical_unit partition,unsigned long blknr,u32 blkcnt, STORAGE_WP_TYPE type)
{
	...
	if ( (type==WP_POWER_ON || type==WP_PERMANENT) && partition == UFS_LU_USER) {
		...
		//Boot partition power on protect, write one partition at the same time
...
static int ufshcd_rpmb_write_data_req_prepare(struct ufs_aio_scsi_cmd *cmd,
        const char *data_buf, u32 size, u32 wr_cnt, u16 addr)
{
	struct rpmb_data_frame *pfrm = NULL;
	int ret = 0;
