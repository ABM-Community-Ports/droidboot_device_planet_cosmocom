...
RETRY:
	...
	if (write == 0) {
		...
		*rdata = GET_WACS2_RDATA(reg_rdata);
		...
		/* Check FSM for IDLE and Write 1 clear to recover FSM to IDLE */
		reg_rdata = WRAP_RD32(PMIC_WRAP_WACS2_RDATA);
		while (wait_for_fsm_idle(reg_rdata)) {
			PWRAPERR("FSM not IDLE, WACS2_RDATA = 0x%x (before)\n", reg_rdata);
			WRAP_WR32(PMIC_WRAP_WACS2_VLDCLR, 1);
			reg_rdata = WRAP_RD32(PMIC_WRAP_WACS2_RDATA);
			PWRAPERR("Recover FSM to IDLE, WACS2_RDATA = 0x%x (after)\n", reg_rdata);
		}
	}
