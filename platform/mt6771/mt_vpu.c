...
int mt_load_vpu(void)
{
	...
	nodeoffset = fdt_node_offset_by_compatible(g_fdt, -1, "mediatek,vpu_core0");  // get vpu node offset
	...
	if (nodeoffset < 0) { // node add sub-node to /
		...
		if (offset < 0) {
			...
			nodeoffset = fdt_add_subnode(g_fdt, offset, "vpu_core0");
			dprintf(INFO, "[%s] nodeoffset %d\n", MODULE_NAME, nodeoffset);
			if (nodeoffset < 0) {
				ret = -EINVAL;
				...
				fdt_setprop_string(g_fdt, nodeoffset, "compatible", "mediatek,vpu_core0");
			}
		}
	}
