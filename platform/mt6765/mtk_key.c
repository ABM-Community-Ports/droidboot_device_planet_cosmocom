...
void get_kpd_dts_info(void)
{
	...
	offset = fdt_path_offset(g_fdt, "/kp@10010000");
	if (offset >= 0) {
		kpd_dts_data.kpd_sw_pwrkey = kpd_fdt_getprop_u32(g_fdt, offset, "mediatek,kpd-sw-pwrkey");
		kpd_dts_data.kpd_hw_pwrkey = kpd_fdt_getprop_u32(g_fdt, offset, "mediatek,kpd-hw-pwrkey");
		kpd_dts_data.kpd_sw_rstkey = kpd_fdt_getprop_u32(g_fdt, offset, "mediatek,kpd-sw-rstkey");
		kpd_dts_data.kpd_hw_rstkey = kpd_fdt_getprop_u32(g_fdt, offset, "mediatek,kpd-hw-rstkey");
		kpd_dts_data.kpd_use_extend_type = kpd_fdt_getprop_u32(g_fdt, offset, "mediatek,kpd-use-extend-type");
		kpd_dts_data.kpd_hw_recovery_key = kpd_fdt_getprop_u32(g_fdt, offset, "mediatek,kpd-hw-recovery-key");
		kpd_dts_data.kpd_hw_factory_key = kpd_fdt_getprop_u32(g_fdt, offset, "mediatek,kpd-hw-factory-key");
		...
		kpd_dts_data.kpd_hw_map_num = kpd_fdt_getprop_u32(g_fdt, offset, "mediatek,kpd-hw-map-num");
		kpd_fdt_getprop_u32_array(g_fdt, offset, "mediatek,kpd-hw-init-map", (char *)kpd_dts_data.kpd_hw_init_map, KPD_NUM_KEYS * 4);
		kpd_dts_init_done = 1;
	} else {
