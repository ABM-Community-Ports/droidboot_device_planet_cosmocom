		   ...
		   void *ramdisk, unsigned ramdisk_sz)
{
	...
	if (g_boot_mode == RECOVERY_BOOT || g_boot_mode == RECOVERY_BOOT2) {
...
#if 1
	ptr = (char *)target_atag_emmcpartnum((unsigned *)buf, g_boot_arg->emmc_part_num);
	ret = fdt_setprop(fdt, offset, "atag,emmcpartnum", buf, ptr - buf);
	if (ret) return FALSE;
#endif
