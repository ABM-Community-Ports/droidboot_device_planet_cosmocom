...
int __attribute__((weak)) aee_mrdump_get_info(struct mrdump_control_block *mcb)
{
	if (mcb == NULL)
		return -1;
...
uint64_t kedump_mem_read(uint64_t data, unsigned long sz, void *buf)
{
...
#ifdef MTK_3LEVEL_PAGETABLE
	{
		...
		if (start >= DRAM_PHY_ADDR) {
			int map_ok = arch_mmu_map(vaddr, vaddr, MMU_MEMORY_TYPE_NORMAL_WRITE_BACK | MMU_MEMORY_AP_P_RW_U_NA, secsize);
			if (map_ok != NO_ERROR) {
				LOG("kedump: arch_mmu_map restore error: map_ok=%d, vaddr=0x%08x in %s\n",
					map_ok, vaddr, __func__);
			}
		}
	}
#endif
...
static int kedump_skip(void)
{
	...
	if (ram_console_reboot_by_cold_reset()) {
		...
		kedump_restore_mem();
		/* a dummy calling here to set reboot reason info */
